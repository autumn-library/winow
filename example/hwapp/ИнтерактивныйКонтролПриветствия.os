...
Процедура ПроверкаПостЗапросаКакОбъект(Ответ, ТелоЗапросаОбъект, ЗаголовкиЗапроса) Экспорт
	...
	Ответ.ТелоТекст = ЗаголовкиЗапроса["Content-Type"] + ТипЗнч(ТелоЗапросаОбъект)// СтрШаблон("%1 %2", ТелоЗапросаОбъект["Имя"], ТелоЗапросаОбъект["Фамилия"]);
КонецПроцедуры

// TODO: Пример для postjsonbodytyped
