...
winow умеет сам заводить серверный спан вокруг обработки каждого HTTP-запроса. Ничего писать для этого не нужно — достаточно включить трассировку в настройках.

### Включение

По умолчанию трассировка выключена, и это не формальность: пока флаг не взведён, SDK не поднимается вовсе. У приложения, которое трассировку не просило, не стартуют экспортёры, не создаются спаны и не появляется никаких накладных расходов.
...
Точка входа должна подключить интеграцию autumn с OpenTelemetry:
...
Адрес коллектора по умолчанию — ```http://localhost:4318``` для ```http/protobuf``` и ```http://localhost:4317``` для ```grpc```. Задать свой можно параметром ```otel.exporter.otlp.endpoint```.

Выключателей на самом деле два, и они независимы:

- ```otel.enabled``` — флаг autumn-opentelemetry. Управляет инструментированием: создавать ли спаны и оборачивать ли желуди. По умолчанию выключен.
- ```otel.sdk.disabled``` — флаг из спецификации OpenTelemetry. Переводит сам SDK в режим заглушки. По умолчанию не взведён.

### Что попадает в спан

Имя спана — метод запроса и шаблон маршрута, например ```GET /calc/{Число1}/multiply/{Число2}```. Именно шаблон, а не конкретный путь: иначе имён операций стало бы столько же, сколько различных URL, и в бэкенде их невозможно было бы группировать. Если запрос не сопоставился ни одному маршруту, именем остаётся один метод.
...
Чтобы передать контекст дальше, при исходящем вызове нужно внедрить его в заголовки запроса:
...
Процедура Оформить(Ответ, ТелоЗапроса) Экспорт
	...
	КоннекторHTTP.Post("http://localhost:3334/склад/резерв", ТелоЗапроса, , ДопПараметры);
...
Желуди самого winow размечены аннотациями инструментирования, поэтому при включённой трассировке спаны появляются не только вокруг запроса, но и вокруг методов контроллеров, менеджеров доступа, отображений и сессий. Это даёт видимость внутри обработки запроса, но и стоит времени на каждом вызове — учитывайте это, если включаете трассировку под нагрузкой.

Серверные спаны winow помечены областью инструментирования ```winow```, спаны методов — областью ```autumn-observed``` от autumn-opentelemetry. По этим именам их можно различать в бэкенде и включать или выключать по отдельности.
