...
winow умеет сам заводить серверный спан вокруг обработки каждого HTTP-запроса. Ничего писать для этого не нужно — достаточно включить телеметрию в настройках.

Трассировка — не единственный сигнал: точки маршрута размечены аннотациями, снимающими метрики, а логи приложения можно отправлять тем же транспортом через аппендер logos. Подробности про сами аннотации, метрики и настройки SDK — в [документации autumn-opentelemetry](https://autumn-library.github.io/autumn-opentelemetry/).
...
Выключателя два, и они независимы:
