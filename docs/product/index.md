...
После запуска приложения станут доступны два адреса:

* `http://localhost:3333/swagger` — страница Swagger UI.
* `http://localhost:3333/swagger/openapi.json` — сама спецификация в формате JSON.
...
`&Минимум` и `&Максимум` сравнивают значение целиком, а не его длину, поэтому им соответствуют `minimum` и `maximum`, а не `minLength` и `maxLength`.
...
| `БазовыйПуть` | `/swagger` | Путь, по которому отдается страница Swagger UI. |
| `ПутьСпецификации` | `/swagger/openapi.json` | Путь, по которому отдается JSON спецификации. |
...
Раздел `servers` — это адрес, к которому Swagger UI приписывает пути при выполнении запроса по кнопке «Try it out». По умолчанию он собирается из имени хоста и порта, которые слушает приложение. Задавайте `АдресСервера` явно, когда приложение доступно не по тому адресу, который слушает: за обратным прокси, под доменным именем, по `https`. Своего TLS у winow нет, поэтому автоматически собранный адрес всегда `http`.
