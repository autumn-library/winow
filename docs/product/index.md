...
* **Тело запроса** — если метод принимает тело (`ТелоЗапросаОбъект`, `ТелоЗапроса` и т.п.) или для параметра указан тип через аннотацию `&Тип`.
* **HTTP-метод** — по умолчанию `get`, а для точек маршрута, читающих тело запроса, — `post`.
