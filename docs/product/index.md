...
* `&Операция(Сводка, Описание, Группы, Устарело, Идентификатор)` — всё, что говорят об операции словами, в одном месте. Устроена так же, как `@Operation` в springdoc: у неё есть параметр на каждое поле операции, и при этом остаются отдельные аннотации. Обе формы равноправны, выбирают ту, что короче в конкретном месте, а значения складываются — группы объединяются, устаревание срабатывает от любой из двух.
