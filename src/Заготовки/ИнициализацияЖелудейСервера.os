...
Процедура ДобавитьЖелудиПриложений(Поделка)
	...
	Лог.Информация(СтрШаблон("Чтение желудей приложения из %1", КаталогСПриложениями));

	СчетчикКлассов = 0;
	
	Файлы = НайтиФайлы(КаталогСПриложениями, "*.os", Истина);
	Для Каждого Файл Из Файлы Цикл
		Лог.Информация(СтрШаблон("Регистрация класса %1", Файл.ИмяБезРасширения));
	...
	Лог.Информация(СтрШаблон("Загружено классов приложения %1", СчетчикКлассов));
...
Процедура ДобавитьЖелудиСервера(Поделка)
	Лог.Информация("Загрузка желудей ядра");
