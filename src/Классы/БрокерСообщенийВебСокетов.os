...
Процедура ВходящееСообщение(Идентификатор, Топик, ДвоичныеДанныеСообщения) Экспорт
	...
	ИначеЕсли ДанныеСообщения.ТипСообщения = 1 Тогда // Текстовый фрейм
		...
		// СформироватьМассивНеобходимыхПараметров ждёт коллекцию с методом Получить: у Структуры его нет,
		// поэтому значения передаются Соответствием — так же, как для точек маршрута HTTP
		Значения = Новый Соответствие();
		Значения.Вставить("Идентификатор", Идентификатор);
		Значения.Вставить("Топик", Топик);
		Значения.Вставить("Сообщение", ДанныеСообщения.Сообщение);

		Параметры = КонструкторыПараметров.СформироватьМассивНеобходимыхПараметров(Обработчик.Действие.Параметры, Значения);
...
Процедура ОбработатьСобытиеПодписки(Топик, Идентификатор, ПодпискиВебСокета)
	...
	Если не ОписаниеПодписки = Неопределено Тогда
		...
		Параметры = КонструкторыПараметров.СформироватьМассивНеобходимыхПараметров(ОписаниеПодписки.Параметры, Значения);
