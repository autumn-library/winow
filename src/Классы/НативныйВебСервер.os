...
Функция ЗапросИзКонтекста(Контекст)
	...
	Запрос.ПолныйПуть = Контекст.Запрос.Путь;
	Если ЗначениеЗаполнено(Контекст.Запрос.СтрокаПараметров) Тогда
		Запрос.ПолныйПуть = Запрос.ПолныйПуть + РаскодироватьСтроку(Контекст.Запрос.СтрокаПараметров, СпособКодированияСтроки.КодировкаURL);
	КонецЕсли;
