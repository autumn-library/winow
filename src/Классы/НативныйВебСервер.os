...
&Пластилин
Перем ОбработчикНативныхЗапросов;
...
Процедура ОбработатьЗапрос(Контекст, СледующийОбработчик) Экспорт // BSLLS:UnusedParameters-off
	...
	Запрос = ОбработчикНативныхЗапросов.ЗапросИзКонтекстаБезТела(Контекст);

	ОбработчикНативныхЗапросов.ОбработатьРазобранныйЗапрос(Запрос, Контекст);
