...
Функция АвторизованныйОтвет(Запрос, Сессия, ОбработчикИПараметры)
	...
	Попытка
		ПередаваемыеПараметрыМетода = Новый Массив;
		ПередаваемыеПараметрыМетода.Добавить(Запрос);
		ПередаваемыеПараметрыМетода.Добавить(Ответ);
		ПередаваемыеПараметрыМетода.Добавить(Сессия);
		
		Рефлектор.ВызватьМетод(	ОбработчикИПараметры.Действие.Желудь, 
								ОбработчикИПараметры.Действие.ИмяМетода, 
								ПередаваемыеПараметрыМетода);
		Ответ.ОбработатьМодель();
	Исключение
