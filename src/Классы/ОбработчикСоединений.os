...
&Пластилин
Перем КонструкторПараметров Экспорт;
...
Функция АвторизованныйОтвет(Запрос, Сессия, ОбработчикИПараметры)
	...
	Попытка
		ПарамтерыТочкиМаршрута = КонструкторПараметров.СформироватьПараметрыТочкиМаршрута(
		...
		Рефлектор.ВызватьМетод(	ОбработчикИПараметры.Действие.Желудь, 
								...
								ПарамтерыТочкиМаршрута);
		Ответ.ОбработатьМодель();
		ОбработатьШаблонКонтрола(Ответ, ОбработчикИПараметры.Действие);
	Исключение
...
Процедура ОбработатьШаблонКонтрола(Ответ, Действие)
