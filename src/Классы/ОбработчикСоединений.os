...
Процедура Обработать(Соединение) Экспорт
	...
	Запрос = РаспарситьЗапрос(ДанныеЗапроса);

	Ответ = СформироватьОтветНаЗапрос(Запрос);

	ДвоичныеДанныеОтвета = ПодготовительОтветов.ДвоичныеДанныеОтвета(Ответ);
...
Функция РаспарситьЗапрос(ДвоичныеДанныеЗапроса)
	Возврат ПарсерЗапросов.ПолучитьЗапросИзДвоичныхДанных(ДвоичныеДанныеЗапроса);
КонецФункции

Функция СформироватьОтветНаЗапрос(Запрос)
	Возврат ОбработчикЗапросов.ПодготовитьОтвет(Запрос);
КонецФункции
