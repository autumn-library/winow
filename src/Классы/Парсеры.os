...
Функция РазделитьДвоичныеДанныеРазделителемВМассив(Знач ДвоичныеДанные, Разделитель, ВКлючатьПустые = Ложь) Экспорт
	...
	НакопленныеБайты = Новый Массив();

	Пока не Чтение.ЧтениеЗавершено Цикл
		ТекБайт = Чтение.Прочитать(1).ПолучитьДвоичныеДанные();
		НакопленныеБайты.Добавить(ТекБайт);
		ДобавитьВМассивСоСмещением(ТекущийПорядокБайт, ТекБайт);

		Если МассивыРавны(БайтыРазделителя, ТекущийПорядокБайт) Тогда

			Партия = Новый Массив;
			Для Счетчик = 0 по НакопленныеБайты.ВГраница() - БайтыРазделителя.Количество() Цикл
				Партия.Добавить(НакопленныеБайты[Счетчик]);
			КонецЦикла;
			НакопленныеБайты = Новый Массив();
			Если Партия.Количество() > 0 Или ВКлючатьПустые Тогда
				Результат.Добавить(СоединитьДвоичныеДанные(Партия));
			КонецЕсли;

		КонецЕсли;
	КонецЦикла;

	Если НакопленныеБайты.Количество() > 0 Тогда
		Результат.Добавить(СоединитьДвоичныеДанные(НакопленныеБайты));
	КонецЕсли;
