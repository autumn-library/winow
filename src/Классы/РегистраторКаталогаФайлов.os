...
// Семафор - сериализует изменения СоответствиеПутей и ЗарегистрированныеФайлы.
//
// Регистрировать файлы могут одновременно несколько рогаток: autumn запускает их
// параллельно в фоновых заданиях, а соответствие и массив к одновременной записи
// не готовы.
//
// Семафор не реентрантный, поэтому критические секции держатся вокруг самих
// изменений и не охватывают вызовы соседних методов.
Перем Семафор;
...
Процедура РегистрацияФайла(РасположениеНаСайте, ПутьНаДиске, ТипКонтента) Экспорт
	...
	ЗапомнитьПутьКФайлу(РасположениеНаСайте, ПутьНаДиске, ТипКонтента);
	...
	// Адрес файла фиксирован, поэтому годится как шаблон маршрута:
	// иначе спаны отдачи статики остались бы без http.route.
	Действие.ШаблонПути = РасположениеНаСайте;
...
Процедура ДобавитьМаршрутДоФайла(Файл, знач КаталогФайлов, знач ПутьНаСайте, знач ТипКонтента)
	...
	Если НЕ ЗапомнитьФайлЕслиНовый(ПутьЮниксСлешами) Тогда
		Возврат;
	КонецЕсли;
...
Процедура ЗапомнитьПутьКФайлу(РасположениеНаСайте, ПутьНаДиске, ТипКонтента)

	Семафор.Захватить();

	Попытка
		СоответствиеПутей.Вставить(
			РасположениеНаСайте,
			Новый Структура("ПутьНаДиске, ТипКонтента", ПутьНаДиске, ТипКонтента));
	Исключение
		Семафор.Освободить();
		ВызватьИсключение;
	КонецПопытки;
...
// Проверяет и помечает файл как зарегистрированный одной операцией:
// раздельные проверка и добавление из параллельных рогаток пропустили бы дубль.
//
// Возвращаемое значение:
//  Булево - Истина, если файл встретился впервые.
//
Функция ЗапомнитьФайлЕслиНовый(ПутьКФайлу)

	Семафор.Захватить();

	Попытка
		ЭтоНовыйФайл = ЗарегистрированныеФайлы.Найти(ПутьКФайлу) = Неопределено;
		Если ЭтоНовыйФайл Тогда
			ЗарегистрированныеФайлы.Добавить(ПутьКФайлу);
		КонецЕсли;
	Исключение
		Семафор.Освободить();
		ВызватьИсключение;
