// Конвейер обработки запроса нативного веб-сервера: превращает контекст Kestrel
// во ВходящийЗапрос, отдаёт его обработчику и записывает ответ обратно в контекст.
//
// Парная реализация для прикладного сервера - ПрикладнойОбработчикЗапросов.
...
//
// Отдельным желудём вынесено из НативныйВебСервер по двум причинам. Во-первых,
// сервер отвечает за жизненный цикл, а не за обработку запроса. Во-вторых,
// напильник оборачивает желудь декоратором, который делегирует вызовы исходному
// объекту: перехватчики срабатывают только на вызовах извне, поэтому шов должен
...
//
Функция РазобратьЗапрос(Контекст) Экспорт
	...
	// Kestrel уже раскодировал путь (всё, кроме %2F), поэтому декодировать его
	// второй раз нельзя: иначе %2520 превратится в пробел вместо %20.
	// Строка параметров приходит как есть, вместе с ведущим "?", и такой же
	// остаётся - по HTTP semantic conventions url.query отдаётся в исходном виде.
	Запрос.Путь = Контекст.Запрос.Путь;
	Запрос.ПолныйПуть = Запрос.Путь + СтрокаПараметров(Контекст);
...
// Запроса без параметров Kestrel отдаёт Неопределено, а не пустую строку.
Функция СтрокаПараметров(Контекст)

	Значение = Контекст.Запрос.СтрокаПараметров;
