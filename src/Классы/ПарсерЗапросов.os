...
Функция ПолучитьОписаниеЗапроса(ЗапросИзКонтекста)
	...
	Для каждого КиЗ Из ЗапросИзКонтекста.Параметры Цикл
		Запрос.ПараметрыИменные.Вставить(КиЗ.Ключ, КиЗ.Значение);
	КонецЦикла;
	...
	Запрос.ПолныйПуть = ЗапросИзКонтекста.Путь + РаскодироватьСтроку(ЗапросИзКонтекста.СтрокаПараметров, 
