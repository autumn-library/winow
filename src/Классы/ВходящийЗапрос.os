...
Функция ТелоЗапросаОбъект() Экспорт
	...
	Если ЗначениеЗаполнено(Тело)
		...
		ТипРезультата
		 = Неопределено;
		Результат = Десериализатор.Десериализовать(Тело, ТипРезультата);
		// TODO: Проверить переданные параметры в Парсер и что там лежит в Настройки.
