...
// Шаблон маршрута, которому сопоставился запрос, например "/calc/{Число1}/multiply/{Число2}".
// Заполняется после маршрутизации и пуст, если подходящий маршрут не нашёлся.
Перем ШаблонМаршрута Экспорт;
